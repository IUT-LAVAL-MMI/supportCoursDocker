...
export class RestResource {
  @observable _url; // URL de la ressource, possiblement paramétriques

  @observable _urlParameters; // valeurs des paramètres de l'URL

  @observable _idField; // nom du champ des données représentant l'id de l'URL (le cas échéant)

  @observable _data; // données de la ressource
  ...
  static async find({
    ...
    return Array.isArray(entriesData) ? entriesData.map((rscData) => new RestResource(url,
      parameters, idField, rscData)) : new RestResource(this.url,
      parameters, idField, entriesData);
  }
}
